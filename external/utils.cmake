include(${CMAKE_CURRENT_SOURCE_DIR}/download_prebuilt_packages_msvc.cmake)
...
set(FIND_LOCAL_SOURCE ${FIND_LOCAL_SOURCE} PARENT_SCOPE)
set(BUILD_EXTERNAL_SOURCE ${BUILD_EXTERNAL_SOURCE} PARENT_SCOPE)
set(BUILD_PACKAGED_SOURCE ${BUILD_PACKAGED_SOURCE} PARENT_SCOPE)
set(DOWNLOAD_PREBUILT_SOURCE ${DOWNLOAD_PREBUILT_SOURCE} PARENT_SCOPE)
...
function(handle_external_package)
    ...
    set(oneValueArgs NAME PREFERRED_SOURCE GIT_REPOSITORY GIT_TAG INCLUDE_TARGET LIBRARY_TARGET_POSIX LIBRARY_TARGET_MSVC_DEBUG LIBRARY_TARGET_MSVC_RELEASE ALTERNATIVE_LOCAL_NAME FORCE_PREFERRED_SOURCE SOURCE_SUBDIR)
    ...
    if (NOT HEP_FORCE_PREFERRED_SOURCE)
    ...
    foreach (SOURCE_OPTION ${ORDERED_SOURCE_OPTIONS})
        if (NOT PACKAGE_READY)
            if (SOURCE_OPTION STREQUAL FIND_LOCAL_SOURCE)
                list(LENGTH HEP_COMPONENTS COMPONENTS_LENGTH)
                list(LENGTH HEP_OPTIONAL_COMPONENTS OPTIONAL_COMPONENTS_LENGTH)
                if(NOT HEP_ALTERNATIVE_LOCAL_NAME)
                    set(HEP_ALTERNATIVE_LOCAL_NAME ${HEP_NAME})
                endif()
                if (HEP_FORCE_PREFERRED_SOURCE)
                    if(COMPONENTS_LENGTH GREATER 0)
                        if(OPTIONAL_COMPONENTS_LENGTH GREATER 0)
                            find_package(${HEP_ALTERNATIVE_LOCAL_NAME} QUIET REQUIRED COMPONENTS ${HEP_COMPONENTS} OPTIONAL_COMPONENTS ${HEP_OPTIONAL_COMPONENTS})
                        else()

                            find_package(${HEP_ALTERNATIVE_LOCAL_NAME} QUIET REQUIRED COMPONENTS ${HEP_COMPONENTS})
                        endif()
                    else()
                        find_package(${HEP_ALTERNATIVE_LOCAL_NAME} QUIET REQUIRED)
                    endif()
                else ()
                    if(COMPONENTS_LENGTH GREATER 0)
                        if(OPTIONAL_COMPONENTS_LENGTH GREATER 0)
                            find_package(${HEP_ALTERNATIVE_LOCAL_NAME} QUIET COMPONENTS ${HEP_COMPONENTS} OPTIONAL_COMPONENTS ${HEP_OPTIONAL_COMPONENTS})
                        else()
                            find_package(${HEP_ALTERNATIVE_LOCAL_NAME} QUIET COMPONENTS ${HEP_COMPONENTS} )
                        endif()
                    else()
                        find_package(${HEP_ALTERNATIVE_LOCAL_NAME} QUIET)
                    endif()
                endif ()
                if (${HEP_ALTERNATIVE_LOCAL_NAME}_FOUND)
                    add_library(${HEP_NAME} INTERFACE IMPORTED GLOBAL)
                    set(${HEP_NAME}_FOUND TRUE PARENT_SCOPE)
                    
                    if(COMPONENTS_LENGTH GREATER 0)
                        set(INTERFACE_INCLUDE_DIRECTORIES)
                        foreach(COMPONENT ${HEP_COMPONENTS})
                            target_link_libraries(${HEP_NAME} INTERFACE ${HEP_ALTERNATIVE_LOCAL_NAME}::${COMPONENT})
                            get_target_property(${COMPONENT}_INTERFACE_INCLUDE_DIRECTORIES ${HEP_ALTERNATIVE_LOCAL_NAME}::${COMPONENT} INTERFACE_INCLUDE_DIRECTORIES)
                        ...
                        foreach(COMPONENT ${HEP_OPTIOHNAL_COMPONENTS})
                            if(TARGET ${COMPONENT})
                                target_link_libraries(${HEP_NAME} INTERFACE ${HEP_ALTERNATIVE_LOCAL_NAME}::${COMPONENT})
                                get_target_property(${COMPONENT}_INTERFACE_INCLUDE_DIRECTORIES ${HEP_ALTERNATIVE_LOCAL_NAME}::${COMPONENT} INTERFACE_INCLUDE_DIRECTORIES)
                                if(${COMPONENT}_INTERFACE_INCLUDE_DIRECTORIES)
                        ...
                        if(INTERFACE_INCLUDE_DIRECTORIES)
                            ...
                            set_target_properties(${HEP_NAME} PROPERTIES INTERFACE_INCLUDE_DIRECTORIES ${INTERFACE_INCLUDE_DIRECTORIES})
                        endif()
                    else()
                        target_link_libraries(${HEP_NAME} INTERFACE ${HEP_ALTERNATIVE_LOCAL_NAME}::${HEP_ALTERNATIVE_LOCAL_NAME})
                    endif()
                    set(PACKAGE_READY TRUE)
                    set(${HEP_UC_NAME}_SOURCE ${SOURCE_OPTION} PARENT_SCOPE)
                else ()
                    message(WARNING "Unable to find the local package for external dependency ${HEP_NAME} (trying name \
                ...
                if (${SOURCE_OPTION} STREQUAL "BUILD_EXTERNAL")
                    ...
                    if (Git_FOUND)
                        externalproject_add(__${HEP_NAME}
                                            ...
                                            SOURCE_SUBDIR ${HEP_SOURCE_SUBDIR}
...
${CMAKE_CURRENT_PREFERRED_SOURCE_DIR}/packaged/${HEP_NAME}. Will try other sources if possible...")
            ...
            elseif(SOURCE_OPTION STREQUAL DOWNLOAD_PREBUILT_SOURCE)
                if(MSVC)
                    download_prebuilt_msvc(${HEP_NAME})
                    set(${HEP_UC_NAME}_SOURCE ${SOURCE_OPTION} PARENT_SCOPE)
                    set(PACKAGE_READY TRUE)
                else()
                    message(FATAL_ERROR "DOWNLOAD_PREBUILT_SOURCE option chosen for external dependency ${HEP_NAME}. This option is not currently supported for CMake generators other than Michrosoft Visual Studio.")
                endif()
            endif ()

...
struct AssignRandomDepthWeightsInRangeFunctor<TVoxel, VoxelBlockHash, MEMORYDEVICE_CUDA>{
	...
	AssignRandomDepthWeightsInRangeFunctor(const Extent2Di& range) : range(range), scale(range.to - range.from){
	...
	__device__
	inline void operator()(TVoxel& voxel){
		...
		if(blockIdx.x > 0 || threadIdx.x > 0 || threadIdx.y > 0 || threadIdx.z > 0) return;
		voxel.w_depth = (uchar)((float)range.from + curand_uniform(&random_states[0]) * scale);
	}
private:

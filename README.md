...
4. CUDA version is super-slow in the master branch. Allocations for spatial hash blocks are not perfect, neither in optimization nor in algorithmic sense. See issues #194 and #195. I'm currently testing that code and weeding out the remaining bugs, but the speedup is going to be x94 or about so (6 fps on the Snoopy masked dataset). The new algorithm will be keeping track of voxel blocks that are utilized in lists, and then launching threads for only the hash entries from the lists. The master-branch version is slow because threads are launched for all entries, and most of them terminate when they discover their hash entry hasn't been allocated (doesn't have associated voxels), causing a huge overhead.

...
struct DepthTracker_CUDA::AccuCell {
	int numPoints;
	float f;
	float g[6];
	float h[6+5+4+3+2+1];
...
struct ITMDepthTracker_KernelParameters {
	DepthTracker_CUDA::AccuCell *accu;
	float *depth;
...
int DepthTracker_CUDA::ComputeGandH(float &f, float *nabla, float *hessian, Matrix4f approxInvPose)
{

...
struct ExtendedTracker_CUDA::AccuCell {
	int numPoints;
	float f;
	float g[6];
	float h[6+5+4+3+2+1];
...
struct ITMExtendedTracker_KernelParameters_Depth {
	ExtendedTracker_CUDA::AccuCell *accu;
	float *depth;
...
ExtendedTracker_CUDA::ExtendedTracker_CUDA(Vector2i imgSize_d,
...
void ExtendedTracker_CUDA::ProjectCurrentIntensityFrame(ITMFloat4Image *points_out,
...
// device functions
template<bool shortIteration, bool rotationOnly, bool useWeights>
__device__ void exDepthTrackerOneLevel_g_rt_device_main(ExtendedTracker_CUDA::AccuCell *accu, float *depth,

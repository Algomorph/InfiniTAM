...
void ExtendedTracker::ComputeDelta(float *step, float *nabla, float *hessian, bool shortIteration) const
{
...
bool ExtendedTracker::HasConverged(float *step) const
{
	for (int i = 0; i < 6; i++)
	{
...
void ExtendedTracker::ApplyDelta(const Matrix4f & para_old, const float *delta, Matrix4f & para_new) const
{
	float step[6];

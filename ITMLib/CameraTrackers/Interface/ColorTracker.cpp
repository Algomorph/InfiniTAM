...
static inline bool minimizeLM(const ColorTracker & tracker, ORUtils::SE3Pose & initialization);
...
void ColorTracker::TrackCamera(CameraTrackingState *trackingState, const View *view)
{
	...
	ORUtils::SE3Pose currentPara(view->calibration_information.trafo_rgb_to_depth.calib_inv * trackingState->pose_d->GetM());
	...
	trackingState->pose_d->SetM(view->calibration_information.trafo_rgb_to_depth.calib * currentPara.GetM());
...
void ColorTracker::ApplyDelta(const ORUtils::SE3Pose & para_old, const float *delta, ORUtils::SE3Pose & para_new) const
{
	float paramVector[6];
...
void ColorTracker::EvaluationPoint::computeGradients(bool hessianRequired)
{
...
ColorTracker::EvaluationPoint::EvaluationPoint(ORUtils::SE3Pose *pos, const ColorTracker *f_parent)
{
	float localF[1];

	this->mPara = pos; this->mParent = f_parent;

	ColorTracker *parent = (ColorTracker *)mParent;
...
static inline double stepQuality(ColorTracker::EvaluationPoint *x, ColorTracker::EvaluationPoint *x2, const float *step, const float *grad, const float *B, int numPara)
{

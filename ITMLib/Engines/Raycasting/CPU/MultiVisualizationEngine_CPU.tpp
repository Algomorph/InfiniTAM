...
void MultiVisualizationEngine_CPU<TVoxel, VoxelBlockHash>::CreateExpectedDepths(const VoxelMapGraphManager<TVoxel, VoxelBlockHash> & sceneManager, const ORUtils::SE3Pose *pose, const Intrinsics *intrinsics, RenderState *_renderState) const
{
	...
	for (int localMapId = 0; localMapId < renderState->indexData_host.numLocalMaps; ++localMapId) 
	{
		...
		std::vector<RaycastingBlock> renderingBlocks(MAX_RENDERING_BLOCKS);
		...
		{
			// fill minmaxData
			const RaycastingBlock & b(renderingBlocks[blockNo]);
...
static void RenderImage_common(const ORUtils::SE3Pose *pose, const Intrinsics *intrinsics, RenderState *_renderState, UChar4Image *outputImage, IRaycastingEngine::RenderImageType type){
...
//#endif
	...
	if ((type == IRaycastingEngine::RENDER_COLOUR_FROM_VOLUME) &&
	    (!TVoxel::hasColorInformation)) type = IRaycastingEngine::RENDER_SHADED_GREYSCALE;
...
template<class TVoxel, class TIndex>
void MultiVisualizationEngine_CPU<TVoxel, TIndex>::RenderImage(const ORUtils::SE3Pose *pose, const Intrinsics *intrinsics, RenderState *_renderState, UChar4Image *outputImage, IRaycastingEngine::RenderImageType type) const
{
	RenderImage_common<TVoxel,TIndex>(pose,intrinsics,_renderState,outputImage,type);
}

template<class TVoxel>
void MultiVisualizationEngine_CPU<TVoxel, VoxelBlockHash>::RenderImage(const ORUtils::SE3Pose *pose, const Intrinsics *intrinsics, RenderState *_renderState, UChar4Image *outputImage, IRaycastingEngine::RenderImageType type) const
{

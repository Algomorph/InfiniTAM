...
#include "Shared/RaycastingEngine_Functors.h"
#include "Shared/RaycastingEngine_Shared.h"
#include "RaycastingEngine_Specialized.tpp"

using namespace ITMLib;

template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::FindVisibleBlocks(VoxelVolume<TVoxel, TIndex>* volume, const ORUtils::SE3Pose* pose,
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
int RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::CountVisibleBlocks(
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::CreateExpectedDepths(
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::RenderImage(
		...
		UChar4Image* output_image, IRaycastingEngine::RenderImageType render_image_type,
		IRaycastingEngine::RenderRaycastSelection raycast_source) const {
	...
	switch (raycast_source) {
		case IRaycastingEngine::RENDER_FROM_OLD_RAYCAST:
			raycast_image = render_state->raycastResult;
			break;
		case IRaycastingEngine::RENDER_FROM_OLD_FORWARDPROJ:
			raycast_image = render_state->forwardProjection;
			break;
		case IRaycastingEngine::RENDER_FROM_NEW_RAYCAST:
	...
	if ((render_image_type == IRaycastingEngine::RENDER_COLOUR_FROM_VOLUME) &&
	    (!TVoxel::hasColorInformation))
		render_image_type = IRaycastingEngine::RENDER_SHADED_GREYSCALE;

	switch (render_image_type) {
		case IRaycastingEngine::RENDER_COLOUR_FROM_VOLUME:{
			RenderFromVolumeFunctor<TVoxel, TIndex, TMemoryDeviceType, IRaycastingEngine::RENDER_COLOUR_FROM_VOLUME>
			...
			break;
		case IRaycastingEngine::RENDER_COLOUR_FROM_NORMAL:{
			RenderFromVolumeFunctor<TVoxel, TIndex, TMemoryDeviceType, IRaycastingEngine::RENDER_COLOUR_FROM_NORMAL>
					functor(*output_image,*raycast_image,*volume,light_source);
			ImageTraversalEngine<TMemoryDeviceType>::template TraversePositionOnly<8, 8>(output_image, functor);
		}
			break;
		case IRaycastingEngine::RENDER_COLOUR_FROM_CONFIDENCE: {
			RenderFromVolumeFunctor<TVoxel, TIndex, TMemoryDeviceType, IRaycastingEngine::RENDER_COLOUR_FROM_CONFIDENCE>
					functor(*output_image,*raycast_image,*volume,light_source);
			ImageTraversalEngine<TMemoryDeviceType>::template TraversePositionOnly<8, 8>(output_image, functor);
		}
			break;
		case IRaycastingEngine::RENDER_SHADED_GREYSCALE_IMAGENORMALS:
			if (intrinsics->FocalLengthSignsDiffer()) {
				RenderFromRaycastFunctor<TMemoryDeviceType, true, IRaycastingEngine::RENDER_SHADED_GREYSCALE_IMAGENORMALS>
				        functor(*output_image,*raycast_image,volume->GetParameters().voxel_size,light_source);
				ImageTraversalEngine<TMemoryDeviceType>::template TraversePositionOnly<8, 8>(output_image, functor);
			} else {
				RenderFromRaycastFunctor<TMemoryDeviceType, false, IRaycastingEngine::RENDER_SHADED_GREYSCALE_IMAGENORMALS>
				        functor(*output_image,*raycast_image,volume->GetParameters().voxel_size,light_source);
				ImageTraversalEngine<TMemoryDeviceType>::template TraversePositionOnly<8, 8>(output_image, functor);
			}
			break;
		case IRaycastingEngine::RENDER_SHADED_GREYSCALE:
		default:{
			RenderFromVolumeFunctor<TVoxel, TIndex, TMemoryDeviceType, IRaycastingEngine::RENDER_SHADED_GREYSCALE>
			        functor(*output_image,*raycast_image,*volume,light_source);
			ImageTraversalEngine<TMemoryDeviceType>::template TraversePositionOnly<8, 8>(output_image, functor);
		}
			break;
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::FindSurface(
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::CreatePointCloud(
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::CreateICPMaps(
		VoxelVolume<TVoxel, TIndex>* volume, const View* view, CameraTrackingState* camera_tracking_state, RenderState* render_state) const {
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::ForwardRender(
...
template<class TVoxel, class TIndex, MemoryDeviceType TMemoryDeviceType>
void RaycastingEngine<TVoxel, TIndex, TMemoryDeviceType>::GenericRaycast(

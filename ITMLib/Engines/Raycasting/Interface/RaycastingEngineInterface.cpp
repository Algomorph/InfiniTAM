...
#include "RaycastingEngineInterface.h"
...
void IRaycastingEngine::DepthToUchar4(UChar4Image *dst, const FloatImage& src)
{
...
void IRaycastingEngine::NormalToUchar4(UChar4Image *dst, const Float4Image& src)
{
	Vector4u *dest = dst->GetData(MEMORYDEVICE_CPU);
...
void IRaycastingEngine::WeightToUchar4(UChar4Image *dst, const FloatImage& src)
{
	Vector4u *dest = dst->GetData(MEMORYDEVICE_CPU);
	const float *source = src.GetData(MEMORYDEVICE_CPU);
	int dataSize = static_cast<int>(dst->size());

...
ViewBuilder_CUDA::ViewBuilder_CUDA(const RGBD_CalibrationInformation& calib) : ViewBuilder(calib) {}
...
__global__ void
convertDisparityToDepth_device(float* depth_out, const short* depth_in, Vector2f disparityCalibParams, float fx_depth, Vector2i imgSize);
...
void ViewBuilder_CUDA::ComputeNormalAndWeights(Float4Image& normal_out, FloatImage& sigma_z_out, const FloatImage& depth_in,

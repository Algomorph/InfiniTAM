...
TraverseTwoArraysWithoutItemIndex_Lambda_device(TData1* data_1, TData2* data_2, TLambda lambda, const unsigned int element_count) {
	TraverseTwoArrays_Generic_device(element_count,
	                                 [&data_1, &data_2, &lambda](const int i_item) { lambda(data_1[i_item], data_2[i_item]); });
}


template<typename TData1, typename TData2, typename TLambda>
__global__ static void TraverseTwoArraysWithIndex_Lambda_device(TData1* data_1, TData2* data_2, TLambda lambda, const unsigned int element_count) {
	TraverseTwoArrays_Generic_device(element_count,
	                                 [&data_1, &data_2, &lambda](const int i_item) { lambda(data_1[i_item], data_2[i_item], i_item); });
}
...
__global__ static void TraverseTwoArraysWithIndex_device(TData1* data_1, TData2* data_2, const unsigned int element_count, TFunctor* functor_device) {
	TraverseTwoArrays_Generic_device(element_count,
	                                 [&data_1, &data_2, &functor_device](const int i_item) {
		                                 (*functor_device)(data_1[i_item], data_2[i_item], i_item);
	                                 });
}
...
__global__ static void TraverseWithPixelCoordinate_Lambda_device(TImageElement1* image_data1,
                                                                 TImageElement2* image_data2,
                                                                 const Vector2i resolution,
                                                                 TLambda lambda) {
	int x = threadIdx.x + blockIdx.x * blockDim.x, y = threadIdx.y + blockIdx.y * blockDim.y;
	if (x >= resolution.x || y >= resolution.y) return;
	int index = x + y * resolution.x;
	lambda(image_data1[index], image_data2[index], x, y);
}

//__DEBUG (remove dead code)
// // CUDA global kernels

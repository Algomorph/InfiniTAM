...
//		pool.join_all();
					...
					output_buffer.get()[i_result++] =
							pool.post_work<ReductionResult<TOutput, VoxelBlockHash>>(
									[&step, &input_buffer, i_block_offset, i_result_in_block]() {
										return TReduceBlockLevelStaticFunctor::reduce(
												input_buffer.get()[i_block_offset + i_result_in_block].get(),
												input_buffer.get()[i_block_offset + i_result_in_block + step].get()
										);
									}
							);
				}
		...
		while (result_count > 1) {
			if (result_count % 2 == 1) {
				std::promise<ReductionResult<TOutput, VoxelBlockHash>> promise;
				promise.set_value(ignored_value);
				input_buffer.get()[result_count] = promise.get_future();
				result_count += 1;
			}
			previous_result_count = result_count;
			result_count >>= 1;
						...
						pool.post_work<ReductionResult<TOutput, VoxelBlockHash>>(
								[&input_buffer, i_input]() {
									return TReduceResultLevelStaticFunctor::reduce(
											input_buffer.get()[i_input].get(),
											input_buffer.get()[i_input + 1].get()
									);
								}
						);
			}
...
//   		pool.join_all();
		...
		ReductionResult<TOutput, VoxelBlockHash> final_result = output_buffer.get()[0].get();

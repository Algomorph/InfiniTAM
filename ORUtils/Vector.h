...
struct Vector4_ {
	union {
		...
		struct {
			T fx, fy, cx, cy;
		}; // camera frustum names for components
		struct {
			T min_x, min_y, max_x, max_y;
		}; // common range names for components
		T values[4];
	};
};
...
public:
	...
	_CPU_AND_GPU_CODE_ Vector2(const Vector2_<T>& v) {
		this->x = v.x;
		this->y = v.y;
	}// copy constructor
	...
	_CPU_AND_GPU_CODE_ inline Vector2<unsigned char> toUChar() const {
		...
		return Vector2<unsigned char>((unsigned char) CLAMP(vi.x, 0, 255), (unsigned char) CLAMP(vi.y, 0, 255));
	}
...
public:
	...
	_CPU_AND_GPU_CODE_ explicit Vector4(const T& t) {
		this->x = t;
		this->y = t;
		this->z = t;
		this->w = t;
	} //Scalar constructor
	_CPU_AND_GPU_CODE_ explicit Vector4(const T* tp) {
		this->x = tp[0];
		this->y = tp[1];
		this->z = tp[2];
		this->w = tp[3];
	} // Construct from array
	_CPU_AND_GPU_CODE_ Vector4(const T v0, const T v1, const T v2, const T v3) {
		this->x = v0;
		this->y = v1;
		this->z = v2;
		this->w = v3;
	} // Construct from explicit values
	_CPU_AND_GPU_CODE_ explicit Vector4(const Vector3_<T>& u, T v0 = T(0)) {
		this->x = u.x;
		this->y = u.y;
		this->z = u.z;
		this->w = v0;
	...
	_CPU_AND_GPU_CODE_ explicit Vector4(const Vector2_<T>& u, T v0 = T(0), T v1 = T(0)) {
		this->x = u.x;
		this->y = u.y;
		this->z = v0;
		this->w = v1;
	...
	_CPU_AND_GPU_CODE_ inline Vector4<unsigned char> toUChar() const {
		Vector4<int> vi = toIntRound();
		return Vector4<unsigned char>((unsigned char) CLAMP(vi.x, 0, 255), (unsigned char) CLAMP(vi.y, 0, 255), (unsigned char) CLAMP(vi.z, 0, 255),
		                              (unsigned char) CLAMP(vi.w, 0, 255));
	}
	...
	_CPU_AND_GPU_CODE_ Vector4<T>& setValues(const T* rhs) {
		this->x = rhs[0];
		this->y = rhs[1];
		this->z = rhs[2];
		this->w = rhs[3];
	...
	// comparison for sorting
	_CPU_AND_GPU_CODE_ friend bool operator<(const Vector4<T>& lhs, const Vector4<T>& rhs) {
		return lhs.w == rhs.w ? (lhs.z == rhs.z ? (lhs.y == rhs.y ? (lhs.x < rhs.x) : lhs.y < rhs.y) : lhs.z < rhs.z) : lhs.w < rhs.w;
	}
...
template<class T>
class Vector6 : public Vector6_<T> {
public:
	typedef T value_type;
	...
	_CPU_AND_GPU_CODE_ explicit Vector6(const T& t) {
		this->values[0] = t;
		this->values[1] = t;
		this->values[2] = t;
		this->values[3] = t;
	...
	_CPU_AND_GPU_CODE_ explicit Vector6(const T* tp) {
		this->values[0] = tp[0];
		this->values[1] = tp[1];
		this->values[2] = tp[2];
		this->values[3] = tp[3];
	...
	_CPU_AND_GPU_CODE_ Vector6(const T v0, const T v1, const T v2, const T v3, const T v4, const T v5) {
		this->values[0] = v0;
		this->values[1] = v1;
		this->values[2] = v2;
		this->values[3] = v3;
	...
	_CPU_AND_GPU_CODE_ explicit Vector6(const Vector4_<T>& u, T v0, T v1) {
		this->values[0] = u.x;
		this->values[1] = u.y;
		this->values[2] = u.z;
		this->values[3] = u.w;
	...
	_CPU_AND_GPU_CODE_ explicit Vector6(const Vector3_<T>& u, T v0, T v1, T v2) {
		this->values[0] = u.x;
		this->values[1] = u.y;
		this->values[2] = u.z;
		this->values[3] = v0;
	...
	_CPU_AND_GPU_CODE_ explicit Vector6(const Vector2_<T>& u, T v0, T v1, T v2, T v3) {
		this->values[0] = u.x;
		this->values[1] = u.y;
		this->values[2] = v0;
		this->values[3] = v1;
		this->values[4] = v2, this->values[5] = v3;
	}

	_CPU_AND_GPU_CODE_ inline Vector6<int> toIntRound() const {
		return Vector6<int>((int) ROUND(this[0]), (int) ROUND(this[1]), (int) ROUND(this[2]), (int) ROUND(this[3]), (int) ROUND(this[4]),
		                    (int) ROUND(this[5]));
	}

	_CPU_AND_GPU_CODE_ inline Vector6<unsigned char> toUChar() const {
		Vector6<int> vi = toIntRound();
		return Vector6<unsigned char>((unsigned char) CLAMP(vi[0], 0, 255), (unsigned char) CLAMP(vi[1], 0, 255),
		                              (unsigned char) CLAMP(vi[2], 0, 255), (unsigned char) CLAMP(vi[3], 0, 255),
		                              (unsigned char) CLAMP(vi[4], 0, 255), (unsigned char) CLAMP(vi[5], 0, 255));
	}
	...
	_CPU_AND_GPU_CODE_ Vector6<T>& setValues(const T* rhs) {
		this[0] = rhs[0];
		this[1] = rhs[1];
		this[2] = rhs[2];
		this[3] = rhs[3];
	...
	// comparison for sorting
	_CPU_AND_GPU_CODE_ friend bool operator<(const Vector6<T>& lhs, const Vector6<T>& rhs) {
		return lhs[5] == rhs[5] ? (lhs[4] == rhs[4] ? (lhs[3] == rhs[3] ? (lhs[2] == rhs[2] ? (lhs[1] == rhs[1] ? (lhs[0] < rhs[0]) : lhs[1] < rhs[1])
...
template<class T, int s>
class VectorX : public VectorX_<T, s> {
public:
	typedef T value_type;
	enum {
		value_size = s
	...
	_CPU_AND_GPU_CODE_ VectorX<T, s>& setValues(const T* rhs) {
		for (int i = 0; i < s; i++) this->values[i] = rhs[i];
		return *this;
	}
...
template<class T>
_CPU_AND_GPU_CODE_ inline T sqr(const T& v) { return v * v; }

// compute_allocated the dot product of two vectors
template<class T>
_CPU_AND_GPU_CODE_ inline typename T::value_type dot(const T& lhs, const T& rhs) {
...
template<class T>
_CPU_AND_GPU_CODE_ inline typename T::value_type length(const T& vec) {
	return sqrt(dot(vec, vec));
}

// return the sum of the provided vector's components
template<class T>
_CPU_AND_GPU_CODE_ inline typename T::value_type sum(const T& vec) {
	typename T::value_type r = 0;
...
template<class T>
_CPU_AND_GPU_CODE_ inline T normalize(const T& vec) {
...
//component wise min
template<class T>
_CPU_AND_GPU_CODE_ inline T minV(const T& lhs, const T& rhs) {
	T rv;
	for (int i = 0; i < lhs.size(); i++)
